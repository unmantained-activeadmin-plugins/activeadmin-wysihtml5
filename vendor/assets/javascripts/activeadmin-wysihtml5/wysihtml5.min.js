...
(function(wysihtml5) {
  var insertElement = function (node_name) {
    return {
      exec: function(composer, command, value) {
        value = typeof(value) === "object" ? value : { src: value };

        var doc     = composer.doc,
            node   = this.state(composer),
            textNode,
            i,
            parent;

        if (node) {
          // node already selected, set the caret before it and delete it
          composer.selection.setBefore(node);
          ...
          // firefox and ie sometimes don't remove the node handles, even though the node got removed
        ...
        composer.selection.insertNode(node);
        if (wysihtml5.browser.hasProblemsSettingCaretAfterImg()) {
          textNode = doc.createTextNode(wysihtml5.INVISIBLE_SPACE);
          composer.selection.insertNode(textNode);
          composer.selection.setAfter(textNode);
        } else {
          composer.selection.setAfter(node);
        }
      },
      ...
      state: function(composer) {
        ...
        nodesInSelection = composer.selection.getNodes(wysihtml5.ELEMENT_NODE, function(node) {
          return node.nodeName === node_name;
        });

        if (nodesInSelection.length !== 1) {
          return false;
        }

        return nodesInSelection[0];
      }
    };
  }

  wysihtml5.commands.insertImage = insertElement("IMG");
  wysihtml5.commands.insertVideo = insertElement("IFRAME");
